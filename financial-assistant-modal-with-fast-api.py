...
class InferenceRequest(BaseModel):
    file_b64: str = Field(
        description="Base64-encoded file contents (PDF, CSV, or XLSX)")
    filename: str = Field(
        description="Original filename including extension e.g. 'statement.pdf'")
    query: str = Field(description="Natural language query about the document")


class NarratorResponse(BaseModel):
    answer: str = Field(
        description="Clear, concise natural language answer to the user's query")
    summary: str = Field(
        description="One-line summary of the key insight e.g. 'Total spend: PHP 45,000'")


data_volume = modal.Volume.from_name(
    "financial-insights-data", create_if_missing=True)
...
class FinancialInsightsEngine:
    ...
    def convert_to_md(self, input_path: str) -> str:
        ...
        if ext not in {".pdf", ".csv", ".xlsx"}:
            raise ValueError(
                f"Unsupported file type: {ext}. Provide a PDF, CSV, or XLSX.")
    ...
    def _run_pipeline(self, input_bytes: bytes, filename: str, query: str) -> dict:
        import os
        import tempfile
        ...
        if ext in {".pdf", ".csv", ".xlsx"}:
            ...
            raise ValueError(
                f"Unsupported file type: {ext}. Provide a PDF, CSV, XLSX, or Markdown file.")
        ...
        if decision.needs_computation:
            ...
            print(f"      {DIVIDER}")
            generated = code_generator_node(
                query, schema, statement, transactions)
            ...
            print(f"      {DIVIDER}")
            exec_result = executor_node(
                query, schema, statement, transactions, generated)
        ...
        return {
            ...
            # ── Step 2 : Extractor ────────────────────────────────
            "statement": statement,
            "transactions": transactions[:100],  # cap at 100 rows
            ...
            # ── Step 4 : Code Generator ───────────────────────────
            "code_generator": {
                "reasoning": generated.reasoning if exec_result else None,
                "code": generated.code if exec_result else None,
            },
    ...
    def run_pipeline(self, input_bytes: bytes, filename: str, query: str) -> dict:
        return self._run_pipeline(input_bytes, filename, query)
