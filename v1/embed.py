"""
Stage 1.5 — Metadata Generation + Pinecone Embedding
------------------------------------------------------
For each chunk in doc_chunks/<pdf_name>/:
1. Generate metadata using LLM (chunk_type, date_range, contains)
2. Embed chunk text using OpenAI text-embedding-3-small
3. Upsert vector + metadata into Pinecone

Usage:
    python stage1_5_embed.py <pdf_name>
    python stage1_5_embed.py 379281148-Sample-Credit-Card-Bill
...
def embed_document(pdf_name: str):
    chunks_dir = CHUNKS_DIR / pdf_name
    if not chunks_dir.exists():
        print(f"[ERROR] No chunks found for '{pdf_name}' at {chunks_dir}")
        sys.exit(1)
    ...
    # Process each chunk
    print(f"[2/3] Generating metadata + embeddings for {len(chunk_files)} chunks...")
    results = []
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python stage1_5_embed.py <pdf_name>")
        print("Example: python stage1_5_embed.py 379281148-Sample-Credit-Card-Bill")
        sys.exit(1)

    embed_document(sys.argv[1])